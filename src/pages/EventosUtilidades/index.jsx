...
export function EventosUtilidades() {
  const scrollContainerRef = useRef(null);

  const [eventosUtilidades, setEventosUtilidades] = useState([])
  ...
  function reset() {
    setCurrentPage(1);
    setEventosUtilidades([]);
  }

  useEffect(()=>{
    
    async function eventosUtilidadesArray(){
      ...
      if (currentPage === 1 && eventosUtilidades.length === 0) {
        const dataTemp = date()
        ...
        const data = await api.post('/eventos/utilidades', body)
  
        setEventosUtilidades(data.data)
      }else{
        
        if((currentPage * itemsPerPage)>=eventosUtilidades.length){
        const dataTemp = date()
        let body = {"pagination" : currentPage,"area": filtroArea,"datainicio":filtroDataInicio?filtroDataInicio:dataTemp,"datafim":filtroDataFim?filtroDataFim:dataTemp}
         
        const data = await api.post('/eventos/utilidades', body)
          
        setEventosUtilidades([...eventosUtilidades,...data.data])
    ...
    eventosUtilidadesArray();

    if (scrollContainerRef.current) {
      scrollContainerRef.current.scrollTop = 0;
    }

  }, [eventosUtilidades, currentPage, itemsPerPage]);
            ...
            <Header style={{ padding: 0, background: colorBgContainer}} >
              ...
              <HeaderTable>
                ...
                <div>
                  ...
                  <form>
                    <ExportToExcel data={eventosUtilidades} fileName={'EventosUtilidades'} id="excel"/>
                    <label>
                      Data de Início:
                      <input
                        type="date"
                ...
                <table className="table minha-classe-personalizada" >
                  ...
                  <tbody>
                    {eventosUtilidades.slice((currentPage*itemsPerPage)-itemsPerPage, currentPage * itemsPerPage).map ((d,i) => (
